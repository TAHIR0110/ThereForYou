...
def send_message(message):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a chatbot also a counsellor, you have to console people when they are sad or depressed. also use most human like accent along with umm, uh and sighs so that it could sound more natural",
            },
...
def convert_to_speech(text):
    ...
    response = client.audio.speech.create(model="tts-1", voice=avaj, input=text)
    ...
    return jsonify(
        {
            ...
            "audio_url": audio_url,
            "depressed_flag": depressed_detector,
            "sad_detector": sad_detector,
        }
    )
